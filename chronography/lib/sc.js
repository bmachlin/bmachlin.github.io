...
function callSpotify(url, options, callback) {
    // set access token if it exists/is still valid
    if(isAccessTokenSet() && !isAccessExpired())
    ...
    // set auth header is we have an access token
    if (AccessTokenSC !== '') {
    ...
    r.onreadystatechange = function() {
        if (this.readyState == XMLHttpRequest.DONE) {
            if(this.status == 200) {
                ...
                processError(url, options, callback, this);
            }
        }
    };
    r.send();
...
function processError(url, options, callback, request) {
...
// client_id = client Id for the Spotify app in use
...
function getArtistMulti(artistIds, callback) {
    ...
    return callSpotify(url, {'ids': artistIds.join(',')}, callback);
}
...
function getArtistAlbums(artistId, options, getAll, callback) {
    ...
    if(options.hasOwnProperty('include_groups')) {
        options.include_groups = options.include_groups.join(',');
    }
    if (getAll) {
...
function getAudioFeaturesMulti(trackIds, callback) {
    ...
    return callSpotify(url, {'ids': trackIds.join(',')}, callback);
}
